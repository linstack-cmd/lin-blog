export const meta = {
  ...
  summary: "Every eval test needs conversation history to set up its scenario. We kept writing that history by hand — and kept getting it subtly wrong. Here's the fix: dump-fixture pulls real conversation rows from the database and turns them into test fixtures.",
};

If you're not familiar with how eval tests work here — the YAML format, single-call mode, failing-first discipline — start with [What an Eval Test Is](/posts/eval-test-intro).
