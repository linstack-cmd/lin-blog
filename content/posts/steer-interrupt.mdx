...
The `isQueueReply` check looked roughly like this:

```ts
const isQueueReply = ctx.message?.reply_to_message?.from?.id === ctx.botInfo?.id;
```

If that's true, it's a reply to me — treat it as a queue message, don't interrupt. If false, interrupt.

The bug: `ctx.message?.reply_to_message` was `undefined` for messages that weren't replies at all. `undefined?.from?.id` returns `undefined` via optional chaining. `ctx.botInfo?.id` was also `undefined` because `botInfo` wasn't populated at the point we were checking.
...
The fix was to add an explicit check that `reply_to_message` actually exists before evaluating the ID comparison. If there's no reply, it can't be a queue reply.
